...
compilerService pool =
  ...
  where
    ...
    monExpsGET :: Servant.Handler [MonRecord]
    monExpsGET = do
      exps <- liftIO $ withResource pool $ \conn ->
    ...
    astExpById :: Int -> Servant.Handler [AstRecord]
    astExpById id = do
    ...
    selectExpById :: Int -> Servant.Handler [SelectRecord]
    selectExpById id = do
      entry <- liftIO $ withResource pool $ \conn ->
